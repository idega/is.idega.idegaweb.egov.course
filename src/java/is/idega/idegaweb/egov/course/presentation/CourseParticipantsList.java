...
public class CourseParticipantsList extends CourseBlock {
	...
	protected Layer getNavigation(IWContext iwc) throws RemoteException {
		...
		script4.append("function changeCourseValues() {\n").append("\tCourseDWRUtil.getCourseMapDWR(setCourseOptions, '" + (getSession().getProvider() != null ? getSession().getProvider().getPrimaryKey().toString() : "-1") + "', DWRUtil.getValue('" + PARAMETER_SCHOOL_TYPE_PK + "'), DWRUtil.getValue('" + PARAMETER_COURSE_TYPE_PK + "'), '" + iwc.getCurrentLocale().getCountry() + "');\n").append("}");
